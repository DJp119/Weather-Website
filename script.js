...
//     } catch (err) {
//         console.error(err);
//         alert('Oops, there was a problem fetching the weather data. Please try again later.');
//         loadingScreen.classList.remove("active");
//     }
// }
...
//     desc.innerText = weatherInfo?.weather?.[0]?.description || '';
//     weatherIcon.src = `http://openweathermap.org/img/w/${weatherInfo?.weather?.[0]?.icon}.png`;
//     temp.innerText = `${(weatherInfo?.main?.temp || 0).toFixed(1)}°C`;
//     windspeed.innertext = `${(weatherInfo?.wind?.speed || 0).toFixed(1)} m/s`;
//     humidity.innertext = `${(weatherInfo?.main?.humidity || 0).toFixed(1)} %`;
//     cloudiness.innerText = `${(weatherInfo?.clouds?.all || 0).toFixed(1)} %`;
// }
...
// //   sessionStorage.setItem("user-coordinates", JSON.stringify(userCoordinates));
// //   fetchUserWeatherInfo(userCoordinates.lat, userCoordinates.lon);
// // }
